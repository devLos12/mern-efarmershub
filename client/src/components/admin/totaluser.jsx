...
const TotalUser = () => {
    ...
    const [currUsers, setCurrUsers] = useState(0);
    ...
    useEffect(() => {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/totaluser?year=${selectedYear}${monthParam}`, {
            method: "GET",
            credentials: "include"
        })
            .then((res) => res.json())
            .then((data) => {
                ...
                setCurrUsers(data.currUsers);
                setPrevUsers(data.prevUsers);
                setChartData(data.chartData);
                setLineColor(data.lineColor);
                setUserGrowthRate(data.userGrowthRate);
            })
