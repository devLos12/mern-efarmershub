...
const Sidebar = () => {
    ...
    const data = {
        admin: [
            ...
            { label: 'Shipping Fee',    link: '/admin/shipping-fee',        icon: 'fa-solid fa-truck',                  source: undefined },
            ...
            <nav className="nav custom-scrollbar px-3"
                ...
                }}>
                {
                    navLinks.map((data, i) => {
                        ...
                        // Render group label
                        if (data.group) {
                            return <NavGroup key={`group-${i}`} label={data.group} role={role} />;
                        }

                        const active = isActive(data.link);

                        return (
                            <div key={i} className="navbar-brand text-dark mt-3 d-block w-100"
                                ...
                                onClick={() => {
                                    if (role === "admin") {
                                        ...
                                        // seller: index 3 = chat support
                                        const sellerNavLinks = navLinks.filter(n => !n.group);
                                        const nonGroupIndex = sellerNavLinks.findIndex(n => n.link === data.link);

                                        if (data.label === 'chat support') {
                                            const sendData = {
                                                receiverId: "unknown",
                                                receiverRole: "admin"
                                            }
