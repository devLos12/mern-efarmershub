...
const OnlinePayment = () => {
    ...
    // ✅ BAGONG STATE: para ipakita ang compression progress
    ...
    const compressImage = async (file) => {
        const options = {
            maxSizeMB: 0.5,          // target max ~500KB — sapat na para sa OCR
            maxWidthOrHeight: 1200,  // resize kung malaki ang dimensions
            useWebWorker: true,      // non-blocking — hindi mag-freeze ang UI
            fileType: "image/jpeg",  // palaging i-convert to JPEG for consistency
            initialQuality: 0.85,    // quality na hindi masyadong grabe ang pagkawala
        };
        return await imageCompression(file, options);
    };

    // ✅ I-call ang backend validate-receipt route
    const validateReceipt = async (file) => {
        const formData = new FormData();
        formData.append("image", file);
        formData.append("paymentMethod", checkoutForm.payment); // "gcash" or "maya"
    ...
    // ✅ UPDATED handleFileChange — compress THEN validate
    const handleFileChange = async (e) => {
        ...
        // Show preview gamit ang original file (para hindi mapansin ng user ang compression)
        ...
        try {
            ...
            // Kung mag-fail ang compression, gamitin na lang ang original
        ...
        // STEP 2: Validate yung compressed file
        setValidating(true);
        try {
            const result = await validateReceipt(compressed);
    ...
    // Disabled state ng file input at browse button
    const isBusy = compressing || validating;
        ...
        <div className="container-fluid p-0 mt-2">
            ...
            <div className="card shadow-sm">
                <div className="card-body p-3">
                    ...
                    {compressing && (
                        <div className="alert alert-secondary py-2 px-3 mb-3 d-flex align-items-center gap-2" style={{ fontSize: "12px" }}>
                            <div className="spinner-border spinner-border-sm" role="status">
                                <span className="visually-hidden">Compressing...</span>
                            </div>
                            <span>Optimizing image size, please wait...</span>
                        </div>
                    )}
                    ...
                    {validating && (
                        <div className="alert alert-info py-2 px-3 mb-3 d-flex align-items-center gap-2" style={{ fontSize: "12px" }}>
                            ...
                            <span>Validating your receipt, please wait...</span>
                        </div>
                    )}

                    {/* ✅ VALIDATION RESULT — success or error badge */}
                    {!isBusy && validationResult && (
                        <div
                    ...
                    {!imgPreview ? (
                        <div className="text-center mb-3">
                            <div className="card p-4 bg-light shadow-sm rounded">
                                ...
                                <label
                                    htmlFor="inputFile"
                                    className={`m-0 mt-2 rounded p-1 px-2 text-capitalize text-light bg-${paymentColor} ${isBusy ? "opacity-50 pe-none" : ""}`}
                                    style={{ fontSize: "13px", cursor: isBusy ? "not-allowed" : "pointer" }}
                                >
                                    {compressing ? "Optimizing..." : validating ? "Validating..." : "Browse file"}
                                </label>
                            </div>
                        </div>
                    ) : (
